...
//Recently uploaded
	   if($UserId){
		...
		$Q3="select * from Media where Type = 'Image' and Media.MediaID in (select Media.MediaID from Media where Sharing='Public' and UploaderID not in (select UserID from Blockedlist where BlockID=$UserId) union select Media.MediaID from Media where Sharing ='Friends' and UploaderID in (select FriendID from Friends where UserID=$UserId) or UploaderID=$UserId and UploaderID not in (select UserID from Blockedlist where BlockID=$UserId) union select Media.MediaID from Media where Sharing='Contacts' and UploaderID in (select ContactID from Contacts where UserID=$UserId) and UploaderID not in (select UserID from Blockedlist where BlockID=$UserId)) order by UploadTime desc Limit 3";
		$Q4="select *,count(*) as cnt from Media inner join MediaStats on Media.MediaID=MediaStats.MediaID where Type='Image' and LikeID!=0 and Media.MediaID in (select Media.MediaID from Media where Sharing='Public' and UploaderID not in (select UserID from Blockedlist where BlockID='$UserId') union select Media.MediaID from Media where Sharing ='Friends' and UploaderID in (select FriendID from Friends where UserID='$UserId') or UploaderID=$UserId and UploaderID not in (select UserID from Blockedlist where BlockID='$UserId') union select Media.MediaID from Media where Sharing='Contacts' and UploaderID in (select ContactID from Contacts where UserID='$UserId') and UploaderID not in (select UserID from Blockedlist where BlockID='$UserId')) group by MediaStats.MediaID order by cnt limit 3";
		...
		//print_r($Q3res);
			...
			$Q3="select * from Media where Type='Image' and Sharing='Public' order by UploadTime desc Limit 3";
			$Q4="select *,count(*) as cnt from Media inner join MediaStats on Media.MediaID=MediaStats.MediaID where Type='Image' and Sharing='Public' and LikeID!=0 group by MediaStats.MediaID order by cnt limit 3";
			$Q5="select * from Media where Type='Image' and Sharing='Public' order by Views desc limit 3";
		}
		$Q3prep=$dbconn->prepare($Q3);
		$Q3prep->execute();
		$Q3res=$Q3prep->fetchAll(PDO::FETCH_ASSOC);
...
<div class="container" style="margin-left:0 ">
      	...
      	?>
				<p>Recommended Images</p><hr>
				<div class="row">
				<?php
				foreach ($Q1res as $key => $value) { 	?>
				<div class="col-md-4">
				<a href="showimage.php?id=<?php echo $value['MediaID'];?>">
				<img src="<?php echo $value['URL']; ?>" alt="Image is not being displayed" style="width:304px;height:228px;"></a>
			...
			foreach ($Q1res as $key => $value) {
				...
				$Q2="select * from Media where Channel='$channel' and Type='Image' and Media.MediaID in (select Media.MediaID from Media where Sharing='Public' and UploaderID not in (select UserID from Blockedlist where BlockID='$UserId') union select Media.MediaID from Media where Sharing ='Friends' and UploaderID in (select FriendID from Friends where UserID='$UserId') or UploaderID=$UserId and UploaderID not in (select UserID from Blockedlist where BlockID='$UserId') union select Media.MediaID from Media where Sharing='Contacts' and UploaderID in (select ContactID from Contacts where UserID='$UserId') and UploaderID not in (select UserID from Blockedlist where BlockID='$UserId'))";
