...
$sql="select * from Media where Type='Image' order by Counter desc";
...
$sql1 = "SELECT * from Media WHERE MediaID='$currMediaId' and Type='Image'";		//changed-Aditya added Type
...
if($UserId!=0){			//Guest user has 0 id
	$UQ1="select * from UserPreferences inner join Media on UserPreferences.MediaID=Media.MediaID where UserID='$UserId' and MediaID='$currMediaId' and Type='Image'";//MediaId=$currMediaId
	...
	if(!empty($rows1)){			//Note the PDO::rowCount() function does not work with Mysql server
		...
		$UQ2="select * from UserPreferences inner join Media on UserPreferences.MediaID=Media.MediaID where UserID='$UserId' and Type='Image' order by ViewTime";
		$UQ2prep=$dbconn->prepare($UQ2);
		$UQ2prep->execute();
		$UQ2res=$UQ2prep->fetchAll(PDO::FETCH_ASSOC);
...
MeTube
	...
	<div class="row">
		...
		<img src="<?php echo $URL; ?>" alt="Image not displayed" width="700" height="400"><br>
		...
		<ul style="list-style-type:none">
			...
			<?php 
			 ...
			 ?>
				...
				<img src="<?php echo $value['URL']; ?>" alt="Image not displayed" width="360" height="176"><br>
