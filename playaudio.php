...
$sql="select * from Media where Type='Audio' order by Counter desc";
...
$sql1 = "SELECT * from Media WHERE MediaID='$currMediaId' and Type='Audio'";
...
$Q7prep->execute();
  //The text (through MediaId stored in $currMediaId) has to be received from playaudio.html
//$currMediaId=4;        //here hard coding currentmediaid to media id of the clicked audio.
...
$UQ1="select * from UserPreferences inner join Media on UserPreferences.MediaID=Media.MediaID where UserID='$UserId' and MediaID='$currMediaId' and Type='Audio'";//MediaId=$currMediaId
	...
	if(!empty($rows1)){			//Note the PDO::rowCount() function does not work with Mysql server
		...
		$UQ2="select * from UserPreferences inner join Media on UserPreferences.MediaID=Media.MediaID where UserID='$UserId' and Type='Audio' order by ViewTime";
		$UQ2prep=$dbconn->prepare($UQ2);
		$UQ2prep->execute();
		$UQ2res=$UQ2prep->fetchAll(PDO::FETCH_ASSOC);
...
<div class="col-md-4">
		...
		<ul style="list-style-type:none">
			...
			<li>
<a href="playaudio.php?id=<?php echo $value['MediaID']; ?>"><?php echo $value['Name']; ?></a>
