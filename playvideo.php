...
$sql="select * from Media where Type='Video' order by Counter desc";
...
$sql1 = "select * from Media where MediaID='$currMediaId' and Type='Video'";		//changed-Aditya added Type
...
if($UserId!=0){			//Guest user has 0 id
	$UQ1="select * from UserPreferences inner join Media on UserPreferences.MediaID=Media.MediaID where UserID='$UserId' and MediaID='$currMediaId' and Type='Video'";//MediaId=$currMediaId
	...
	if(!empty($rows1)){			//Note the PDO::rowCount() function does not work with Mysql server
		...
		$UQ2="select * from UserPreferences inner join Media on UserPreferences.MediaID=Media.MediaID where UserID='$UserId' and Type='Video' order by ViewTime";
		$UQ2prep=$dbconn->prepare($UQ2);
		$UQ2prep->execute();
		$UQ2res=$UQ2prep->fetchAll(PDO::FETCH_ASSOC);
...
MeTube
	...
	<div class="row">
		...
		<h1><?php echo $nametag; ?></h1>
				<video id="video1" src="<?php echo $first ;?>" type="video/mp4" width="700" height="400" controls="controls">
		</video>
		<br><br>
		...
		<ul class="list-unstyled video-list-thumbs row">
			...
			<?php 
			 ...
			 ?>
				...
				<video id="video" src="<?php echo $value['URL']; ?>" type="video/mp4" width="360" height="176" class="img-responsive">
			</video>
